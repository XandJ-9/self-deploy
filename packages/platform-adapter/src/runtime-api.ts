...
export function ensureRuntimeApi(): void {
  ...
  document.body.innerHTML =
    ...
    '<p>macOS Electron 开发请执行 <code>npm run dev:mac</code>；Windows Tauri 开发请执行 <code>npm run dev:win</code>。</p>' +
    '</div>';
  throw new Error('window.api missing — open via desktop runtime, not a browser tab.');
}
