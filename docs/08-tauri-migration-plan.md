...
| 凭据平台差异 | 当前分支仅验证 Windows DPAPI 行为；其他平台不在本阶段范围 |
| 双运行时维护成本 | T7 后默认入口切换到 Tauri；Electron 仅保留 `legacy:*` 回退命令，后续可独立删除 |

## 开发命令

```bash
npm run dev          # Tauri 开发，需要 cargo/rustc
npm run dev:tauri    # 同 npm run dev
npm run lint         # TypeScript 检查
npm run build        # Tauri 打包，需要 cargo/rustc
